...
class TransactionServiceConcurrencyTest {
  ...
  void setUpDatabase() {
    ...
    jwt1 = Jwt.withTokenValue("token1").header("alg", "none").claim("userId", user1.getId())
        .claim("sub", user1.getEmail()).build();
    jwt2 = Jwt.withTokenValue("token2").header("alg", "none").claim("userId", user2.getId())
        .claim("sub", user2.getEmail()).build();
  ...
  void executeTransfer_ConcurrentTransfers_PreventsLostUpdates() throws InterruptedException {
    ...
    TransferRequest request = new TransferRequest(senderAcc, receiverAcc, transferAmount,
        "Concurrent Transfer");
    ...
    // Then
    AccountBalance updatedSenderBalance = balanceRepository.findLatestBalance(senderAcc)
        .orElseThrow();
    AccountBalance updatedReceiverBalance = balanceRepository.findLatestBalance(receiverAcc)
        .orElseThrow();
  ...
  void executeTransfer_ConcurrentOverdraft_FailsGracefully() throws InterruptedException {
    ...
    assertThat(successCount.get()).isEqualTo(
        5); // Only 5 transfers of $200 can fit in $1000 balance
  ...
  void executeTransfer_CrossTransfers_DoesNotDeadlock() throws Exception {
    ...
    String senderAcc = "PK1000000001";
    String receiverAcc = "PK2000000002";
    TransferRequest req1 = new TransferRequest(senderAcc, receiverAcc, transferAmount,
        "Overdraft Attempt");
    TransferRequest req2 = new TransferRequest(receiverAcc, senderAcc, transferAmount,
        "Overdraft Attempt");
