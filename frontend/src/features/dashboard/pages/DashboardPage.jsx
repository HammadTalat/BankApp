...
import { formatAccountNumber } from "../../../shared/utils/formatAccountNumber.js";
...
export const DashboardPage = () => {
    ...
    useEffect(() => {
        ...
        const loadDashboardData = async () => {
            setLoading(true);
            try {
                const [accountRes, balanceRes, txRes] = await Promise.allSettled([
                ...
                if (accountRes.status === "fulfilled" && accountRes.value) {
                    setAccountDetails({
                        accountNumber: accountRes.value.accountNumber || "",
                        status: accountRes.value.status || "ACTIVE",
                    });
                }

                if (balanceRes.status === "fulfilled" && balanceRes.value?.amount !== undefined) {
                    setBalance(balanceRes.value.amount);
                }
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                ...
                <div className="lg:col-span-2 bg-[#2563EB] rounded-2xl p-8 text-white flex flex-col justify-between shadow-sm">
                    <div>
                        <div className="flex items-center justify-between">
                            ...
                            <button
                                ...
                                className="p-1.5 rounded-lg bg-white/10 hover:bg-white/20 text-blue-100 hover:text-white transition-colors disabled:opacity-50 cursor-pointer"
                            >
                                <svg
                                    className={`w-4 h-4 ${
                    ...
                    <div className="flex gap-4 mt-8">
                        <button
                            ...
                            className="bg-white text-blue-600 px-6 py-2.5 rounded-lg text-sm font-semibold hover:bg-blue-50 transition-colors shadow-sm cursor-pointer"
                        >
                            Transfer Money
                        </button>
                        <button
                            onClick={() => navigate(ROUTES.ACCOUNT_TRANSACTIONS)}
                            className="bg-white/20 text-white border border-white/30 px-6 py-2.5 rounded-lg text-sm font-semibold hover:bg-white/30 transition-colors cursor-pointer"
                        >
                            View Transactions
                        </button>
                    </div>
                ...
                <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm flex flex-col justify-between">
                    <div>
                        ...
                        <p className="text-xl font-bold text-gray-900 mt-2 tracking-wide font-mono">
                            {accountDetails.accountNumber ? formatAccountNumber(accountDetails.accountNumber) : "Loading..."}
                        </p>
                    ...
                    <div className="mt-6">
                        <button
                            ...
                            className="w-full bg-gray-50 hover:bg-gray-100 text-gray-700 border border-gray-200 py-2.5 px-4 rounded-lg text-xs font-semibold transition-colors text-center disabled:opacity-50 cursor-pointer"
                        >
